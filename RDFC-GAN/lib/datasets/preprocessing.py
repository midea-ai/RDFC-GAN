...
class Normalize:
    ...
    def __call__(self, sample):
        ...
        if 'gt_normal' in sample:
            gt_normal = sample['gt_normal']
            gt_normal = gt_normal / 255
            gt_normal = torchvision.transforms.Normalize(
                mean=self._rgb_mean, std=self._rgb_std)(gt_normal)
            sample['gt_normal'] = gt_normal
...
class ToTensor:
    ...
    def __call__(self, sample):
        ...
        if 'gt_normal' in sample:
            ...
            sample['gt_normal'] = gt_normal
            sample['normal_masks'] = norm_valid_mask

        if 'labels' in sample:
            label = sample['labels']
            sample['labels'] = torch.from_numpy(label).long()
...
class RandomRescale:
    ...
    def __call__(self, sample):
        ...
        if 'gt_normal' in sample:
            gt_normal = sample['gt_normal']
            gt_normal = cv2.resize(gt_normal, (target_width, target_height),
                            interpolation=cv2.INTER_LINEAR)
            sample['gt_normal'] = gt_normal
...
class RandomCrop:
    ...
    def __call__(self, sample):
        ...
        if h <= self.crop_height or w <= self.crop_width:
            ...
            if 'gt_normal' in sample:
                gt_normal = sample['gt_normal']
                gt_normal = gt_normal[i:i + self.crop_height, j:j + self.crop_width, :]
                sample['gt_normal'] = gt_normal
