...
from ..rdf_gan_generator.encoder_decoder.encoder_decoder import (
    EncoderDecoder, conv_bn_relu)
from ..rdf_gan_generator.model_utils import AdaptiveInstanceNorm
from ..rdf_gan_generator.nlspn.nlspn_model import NLSPNRefineModule


class DCVGANGenerator(nn.Module, ABC):
    def __init__(self,
                 ...
                 use_nlpsn_refine=False,
                 nlspn_configs=None):
        super(DCVGANGenerator, self).__init__()

        self.use_nlpsn_refine =  use_nlpsn_refine
        ...
        if fuse_depth_in_rgb_decoder == 'AdaIN':
            ...
            self.fuse_layer2 = AdaptiveInstanceNorm(in_channel=rgb_channels_decoder[0] + rgb_channels_encoder[-2] if\
                ...
                self.rgb_skip_connection_type == 'concat' else depth_channels_decoder[0],
                                                    weighting=adain_weighting)
            self.fuse_layer3 = AdaptiveInstanceNorm(in_channel=rgb_channels_decoder[1] + rgb_channels_encoder[-3] if \
                ...
                self.rgb_skip_connection_type == 'concat' else depth_channels_decoder[1],
                                                    weighting=adain_weighting)
            self.fuse_layer4 = AdaptiveInstanceNorm(in_channel=rgb_channels_decoder[2] + rgb_channels_encoder[-4] if \
                ...
                self.rgb_skip_connection_type == 'concat' else depth_channels_decoder[2],
                                                    weighting=adain_weighting)
            self.fuse_layer5 = AdaptiveInstanceNorm(in_channel=rgb_channels_decoder[3] + rgb_channels_encoder[-5] if \
                ...
                self.rgb_skip_connection_type == 'concat' else depth_channels_decoder[3],
                                                    weighting=adain_weighting)
    ...
    def forward(self, rgb, depth):
        ...
        # encoder layer 1  [1/1]   228 x 304
        rgb_fe1 = self.rgb_branch_en1(rgb)

        depth_fe1_rgb = self.depth_branch_en1_rgb(rgb)
        depth_fe1_depth = self.depth_branch_en1_depth(depth)
        depth_fe1 = torch.cat([depth_fe1_rgb, depth_fe1_depth], dim=1)

        # encoder layer 2  [1/1]  228 x 304
